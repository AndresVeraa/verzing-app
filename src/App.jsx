...
const apiKey = ""; // La clave se proporciona automáticamente en el entorno de ejecución

/**
 * Función central para llamadas a la IA con manejo de errores y reintentos.
 */
  ...
  <section className="relative min-h-screen flex items-center pt-20 px-6 bg-[#FDFCFB] overflow-hidden">
    <div className="max-w-7xl mx-auto w-full grid md:grid-cols-2 gap-16 items-center text-left">
      <div className="animate-in fade-in slide-in-from-bottom-8 duration-1000">
        ...
        <h1 className="text-7xl md:text-8xl font-black tracking-tighter leading-[0.85] mb-10 uppercase">
        ...
        <div className="flex gap-4">
          ...
          <button 
            ...
            className="border-2 border-black px-10 py-5 text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-black hover:text-white transition-all"
          >
            ✨ Probar IA
          </button>
        </div>
...
const ProductModal = ({ product, onClose }) => {
  ...
  const handleGetStyleAdvice = async () => {
    setLoadingAI(true);
    const system = "Eres un estilista premium. Sugiere 3 outfits cortos (Casual, Urban, Sport) para estos sneakers. Sé conciso y elegante en español.";
    const prompt = `Zapatillas: ${product.name} (${product.vibe}).`;
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-5xl rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col md:flex-row animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto no-scrollbar">
        ...
        <div className="md:w-1/2 p-12 flex flex-col text-left">
          ...
          {/* AI Advisor Card */}
          <div className="mb-8 p-6 bg-amber-50 rounded-3xl border border-amber-100 relative">
            <h3 className="text-[10px] font-black uppercase tracking-widest text-amber-800 mb-4 flex items-center gap-2">
              <Sparkles size={12} /> Conserje de Estilo AI
            </h3>
            <div className="text-xs text-amber-900 leading-relaxed mb-4 min-h-[40px]">
              {loadingAI ? (
                <span className="animate-pulse italic">Analizando tendencias para ti...</span>
              ) : (
                aiAdvice || "¿Cómo combinarías estos Verzing? Deja que nuestra IA te sugiera el look perfecto."
              )}
            </div>
            {!aiAdvice && !loadingAI && (
              <button 
                onClick={handleGetStyleAdvice}
                className="bg-amber-600 text-white px-6 py-2 rounded-full text-[9px] font-bold uppercase tracking-widest hover:bg-amber-700 transition-all"
              >
                Generar Consejos
              </button>
            )}
          ...
          <div className="mb-8 bg-neutral-50 p-6 rounded-3xl border border-neutral-100">
...
const AIAssistant = ({ isOpen, onClose }) => {
  const [messages, setMessages] = useState([
    { role: 'bot', text: '¡Hola! Soy tu asistente de Verzing. ¿Buscas algún modelo en especial o necesitas ayuda con tu estilo?' }
  ]);
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-lg rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col h-[600px] animate-in slide-in-from-bottom-4 duration-300">
        <div className="p-8 border-b border-neutral-100 flex justify-between items-center text-left">
          <div>
            <h3 className="text-2xl font-black uppercase tracking-tighter">Asistente AI ✨</h3>
            <p className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest">Experto en Sneakers</p>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-neutral-50 rounded-full"><X size={20} /></button>
        </div>
        ...
        <div ref={chatRef} className="flex-1 overflow-y-auto p-8 space-y-6 no-scrollbar">
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in duration-300`}>
              <div className={`max-w-[85%] p-4 rounded-2xl flex gap-3 text-left ${m.role === 'user' ? 'bg-black text-white rounded-tr-none' : 'bg-neutral-50 text-neutral-800 rounded-tl-none border border-neutral-100'}`}>
                <div className="shrink-0 mt-1">{m.role === 'bot' ? <Bot size={14} /> : <User size={14} />}</div>
          ...
          {isTyping && (
            <div className="flex justify-start">
              <div className="bg-neutral-50 p-4 rounded-2xl border border-neutral-100 italic text-[10px] text-neutral-400 animate-pulse">
        ...
        <div className="p-6 bg-neutral-50 border-t border-neutral-100 flex gap-3">
          <input 
            ...
            placeholder="¿Qué par me recomiendas para..."
            className="flex-1 bg-white border border-neutral-200 rounded-2xl px-6 py-4 text-xs focus:outline-none focus:border-amber-600 transition-colors shadow-sm"
          />
          <button 
            onClick={handleSend}
            className="bg-black text-white p-4 rounded-2xl hover:bg-amber-600 transition-all shadow-lg"
          >
            <Send size={18} />
          </button>
        </div>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#FDFCFB] text-[#1A1A1A] font-sans selection:bg-amber-100 selection:text-amber-900 overflow-x-hidden">
      <style>{styles}</style>
      <Navbar wishlistCount={0} onOpenAssistant={() => setIsAssistantOpen(true)} />
      <Hero onOpenAssistant={() => setIsAssistantOpen(true)} />
      
      {/* Sección del Catálogo con cabecera Sticky */}
      <section id="catalog" className="py-32 px-6">
        <div className="max-w-7xl mx-auto">
          {/* Cabecera pegajosa (Sticky) - Ajustada al h-20 de la Navbar */}
          <div className="sticky top-20 z-40 bg-[#FDFCFB]/95 backdrop-blur-md py-8 border-b border-black/5 flex flex-col md:flex-row justify-between items-end gap-10 mb-20 text-left">
            <div>
              <h2 className="text-6xl font-black tracking-tighter mb-4 uppercase leading-none">The Drop</h2>
            ...
            <div className="flex flex-wrap gap-2 w-full md:w-auto no-scrollbar overflow-x-auto pb-2 md:pb-0">
              {['all', 'Streetwear', 'Retro', 'Limited'].map(vibe => (
                <button 
                  ...
                  className={`px-8 py-3.5 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all border-2 ${activeVibe === vibe ? 'bg-black text-white border-black shadow-lg' : 'border-neutral-100 hover:border-black'}`}
                >
                  {vibe === 'all' ? 'Todos' : vibe}
                </button>
              ))}
